...
3. 按全局目标时间分成三个区块，排除预测标签跨区块的窗口；风机不分别随机分块。
   前两个区块拟合候选策略，最后一个较晚区块用于稀疏事件选择：逐次尝试关闭整个事件族，
   仅当实际路由 MAE 改善时关闭。这个训练区块已参与选择，不能当作独立评估集。
4. 对每个事件/组合、历史趋势类别和预测步长，比较固定修正比例
   `[0, 0.25, 0.5, 1]`，使用与汇报一致的物理功率裁剪计算绝对误差收益。
   分数为区块平均收益减去 0.25 倍区块收益标准差，且需多数区块收益为正。
   每个拟合区块默认至少 32 个可用窗口；趋势细组不足时回退到同事件/步长的全趋势统计。
   随后重放实际推理路由，按整套策略的 MAE 移除在事件竞争中造成负收益的事件/趋势组，
   并检查整体修正幅度。训练后段进一步检验关闭整个事件族是否改善最终路由。
   若训练后段最终没有正收益，整套策略不介入。
   该规则是经验稳定性校准，不是统计置信保证。
5. 推理查询“当前可观测趋势 + 物理事件支持 + 预测步长”对应的比例/收益，
...
完成后关注 `CALIBRATED_MAE_KW` 和 `GAIN_VS_TREND_KW`（正值才改善），以及
`TRAIN_HOLDOUT_GAIN_BEFORE_EVENT_SELECTION_KW`、`TRAIN_HOLDOUT_JOINT_GAIN_KW`
和 `TRAIN_HOLDOUT_REMOVED_EVENT_FAMILIES`。后两个值来自参与事件选择的训练区块，
不作为独立测试成绩。
如果训练后段接受策略，但验证集依然不提升，说明训练收益没有泛化，不能据此选用新模型。
