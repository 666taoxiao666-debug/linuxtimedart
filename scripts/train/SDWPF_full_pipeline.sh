...
if [[ "${SPLIT}" == "rolling_holdout" ]]; then
    ...
    echo "CONFIRM_FINAL_EVAL=1 MODEL=${MODEL} PROMPT_ROUTER=${PROMPT_ROUTER} UTILITY_WIKI=${UTILITY_WIKI} UTILITY_GATE_TEMPERATURE=${UTILITY_GATE_TEMPERATURE} UTILITY_MIN_GAIN=${UTILITY_MIN_GAIN} UTILITY_INTERVENTION_FLOOR=${UTILITY_INTERVENTION_FLOOR} SCENE_WIKI_CONFIG='${SCENE_WIKI_CONFIG}' SCENE_WIKI_EMBEDDINGS='${SCENE_WIKI_EMBEDDINGS}' SCENE_WIKI_TOP_K=${SCENE_WIKI_TOP_K} SCENE_WIKI_TEMPERATURE=${SCENE_WIKI_TEMPERATURE} SCENE_WIKI_RULE_WEIGHT=${SCENE_WIKI_RULE_WEIGHT} SCENE_WIKI_PROMPT_GATE_INIT=${SCENE_WIKI_PROMPT_GATE_INIT} SCENE_WIKI_ACTIVATION_THRESHOLD=${SCENE_WIKI_ACTIVATION_THRESHOLD} SCENE_WIKI_CONFIDENCE_POWER=${SCENE_WIKI_CONFIDENCE_POWER} SEED=${SEED} FOLD=${FOLD} N_FOLDS=${N_FOLDS} SPLIT=${SPLIT} PRED_LEN=${PRED_LEN} RATED_POWER=${RATED_POWER} GPU=${GPU} FINETUNE_CHECKPOINT='${CHECKPOINT}' bash scripts/eval/SDWPF_logged_final_eval.sh"
else
