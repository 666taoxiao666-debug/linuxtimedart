...
if [[ "${SPLIT}" == "rolling_holdout" ]]; then
    ...
    echo "CONFIRM_FINAL_EVAL=1 MODEL=${MODEL} PROMPT_ROUTER=${PROMPT_ROUTER} SCENE_WIKI_CONFIG='${SCENE_WIKI_CONFIG}' SCENE_WIKI_EMBEDDINGS='${SCENE_WIKI_EMBEDDINGS}' SCENE_WIKI_TOP_K=${SCENE_WIKI_TOP_K} SCENE_WIKI_TEMPERATURE=${SCENE_WIKI_TEMPERATURE} SCENE_WIKI_RULE_WEIGHT=${SCENE_WIKI_RULE_WEIGHT} SCENE_WIKI_PROMPT_GATE_INIT=${SCENE_WIKI_PROMPT_GATE_INIT} SEED=${SEED} FOLD=${FOLD} N_FOLDS=${N_FOLDS} SPLIT=${SPLIT} PRED_LEN=${PRED_LEN} RATED_POWER=${RATED_POWER} GPU=${GPU} FINETUNE_CHECKPOINT='${CHECKPOINT}' bash scripts/eval/SDWPF_logged_final_eval.sh"
else
