...
export UTILITY_ADAPTER_MODE="${UTILITY_ADAPTER_MODE:-legacy}"
...
export UTILITY_LOSS_WEIGHT="${UTILITY_LOSS_WEIGHT:-0.1}"
export UTILITY_DECISION_LOSS_WEIGHT="${UTILITY_DECISION_LOSS_WEIGHT:-0.2}"
export UTILITY_CANDIDATE_LOSS_WEIGHT="${UTILITY_CANDIDATE_LOSS_WEIGHT:-0.5}"
export UTILITY_RANKING_LOSS_WEIGHT="${UTILITY_RANKING_LOSS_WEIGHT:-0.2}"
export UTILITY_RANKING_MARGIN="${UTILITY_RANKING_MARGIN:-0.01}"
UTILITY_ADAPTER_WARMUP_EPOCHS="${UTILITY_ADAPTER_WARMUP_EPOCHS:-}"
export UTILITY_LEARNING_RATE="${UTILITY_LEARNING_RATE:-0.00005}"
export UTILITY_GATE_TEMPERATURE="${UTILITY_GATE_TEMPERATURE:-0.25}"
export UTILITY_MIN_GAIN="${UTILITY_MIN_GAIN:-0.005}"
export UTILITY_TARGET_EPS="${UTILITY_TARGET_EPS:-0.05}"
export UTILITY_INTERVENTION_FLOOR="${UTILITY_INTERVENTION_FLOOR:-0.25}"
export PRED_LEN="${PRED_LEN:-12}"
export FOLDS="${FOLDS:-0}"
export SEEDS="${SEEDS:-2024}"
export FREEZE_NON_UTILITY="${FREEZE_NON_UTILITY:-0}"
if [[ -z "${UTILITY_ADAPTER_WARMUP_EPOCHS}" ]]; then
    if [[ "${FREEZE_NON_UTILITY}" == "1" ]]; then
        UTILITY_ADAPTER_WARMUP_EPOCHS=2
    else
        UTILITY_ADAPTER_WARMUP_EPOCHS=0
    fi
fi
...
LOG_PARAMETERS="reuse_h${PRED_LEN}_folds${FOLDS// /-}_seeds${SEEDS// /-}_freeze${FREEZE_NON_UTILITY}_warm${UTILITY_ADAPTER_WARMUP_EPOCHS}_ep${TRAIN_EPOCHS}_pat${PATIENCE}_blr${LEARNING_RATE}_nlr${NEW_MODULE_LEARNING_RATE}_ulr${UTILITY_LEARNING_RATE}_ulw${UTILITY_LOSS_WEIGHT}_udlw${UTILITY_DECISION_LOSS_WEIGHT}_uclw${UTILITY_CANDIDATE_LOSS_WEIGHT}_urlw${UTILITY_RANKING_LOSS_WEIGHT}_ugt${UTILITY_GATE_TEMPERATURE}_umg${UTILITY_MIN_GAIN}_uif${UTILITY_INTERVENTION_FLOOR}"
LOG_PARAMETERS="adapter${UTILITY_ADAPTER_MODE}_ecap${UTILITY_EVENT_MAX_SCALE}_ccap${UTILITY_COMPOSITION_MAX_SCALE}_${LOG_PARAMETERS}"
